...
# 1. Creer le NACL et stocker la reponse dans nacl.json
#create_nacl "$1"

# 2. Extraire le NetworkAclId depuis nacl.json
#NACL_ID=$(python3 -c "import json,sys; print(json.load(open('nacl.json'))['NetworkAcl']['NetworkAclId'])")
#echo "NACL ID recupere : $NACL_ID"

# 3. Creer la regle entrante avec le NACL ID
